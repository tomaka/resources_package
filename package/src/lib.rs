...
impl Package {
    ...
    pub fn find<P>(&self, resource: P) -> Option<&'static [u8]> where P: AsPath {
        let resource = resource.as_path();
...
impl<'a> Iterator for Iter<'a> {
    ...
    fn next(&mut self) -> Option<(PathBuf, &'static [u8])> {
        match self.0.next() {
            None => None,
            Some(&(path, content)) => Some((PathBuf::new(&String::from_utf8(path.to_vec()).unwrap()), content))
        }
    }
}
